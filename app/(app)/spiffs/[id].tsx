...
export default function SpiffDetailScreen() {
  ...
  useSetHeaderOptions(useMemo(() => ({ title: "Spiff", showBack: true }), []));
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        getErrorMessage(error, "Failed to load spiff. Please try again."),
      );
    } finally {
      setLoading(false);
    }
  ...
  const openAttachment = useCallback(
    (url: string | null | undefined, title: string) => {
      if (!url?.trim()) return;
