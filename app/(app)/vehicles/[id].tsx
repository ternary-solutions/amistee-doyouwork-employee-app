...
function getStatusStyle(status: boolean | null | undefined): 'ok' | 'out' | 'needs' {
  if (status === true) return 'ok';
...
export default function VehicleDetailScreen() {
  ...
  const openDocument = useCallback((url: string | null | undefined, docTitle: string) => {
    if (!url?.trim()) return;
  ...
  return (
    ...
    >
      ...
      {/* New Request CTA */}
      <Pressable
        style={({ pressed }) => [styles.newRequestBtn, pressed && { opacity: 0.9 }]}
        ...
        <View style={styles.detailsCard}>
          ...
          <DetailRow
            ...
            value={
              vehicle.status !== undefined && vehicle.status !== null ? (
                <View style={[styles.statusBadge, statusStyle === 'ok' ? styles.statusOk : statusStyle === 'out' ? styles.statusOut : styles.statusNeeds]}>
                  <Text style={styles.statusBadgeText}>{getStatusLabel(vehicle.status)}</Text>
                </View>
              ) : (
                '—'
              )
          ...
          <View style={styles.docSection}>
            ...
            {vehicle.insurance_doc ? (
              <Pressable
                style={({ pressed }) => [styles.docBtn, pressed && { opacity: 0.8 }]}
                onPress={() => openDocument(vehicle.insurance_doc, 'Insurance Document')}
              >
            ...
            {vehicle.registration_doc ? (
              <Pressable
                style={({ pressed }) => [styles.docBtn, pressed && { opacity: 0.8 }]}
                onPress={() => openDocument(vehicle.registration_doc, 'Registration Document')}
              >
                <Ionicons name="document-text" size={18} color={primary} />
                <Text style={styles.docBtnText}>View Registration Document</Text>
              </Pressable>
            ) : null}
...
const styles = StyleSheet.create({
  ...
  headerTitle: { ...typography.sectionTitle, fontSize: 22, color: foreground, marginBottom: spacing.md },
  photoWrap: { marginBottom: spacing.sm },
  photo: { width: 100, height: 100, borderRadius: 50, borderWidth: 3, borderColor: card },
  photoPlaceholder: {
    width: 100,
    height: 100,
    borderRadius: 50,
  ...
  newRequestBtnText: { fontSize: 16, fontWeight: '600', color: primaryForeground },
  tabRow: { flexDirection: 'row', backgroundColor: muted, borderRadius: radius.full, padding: 4, marginBottom: spacing.lg },
  tab: { flex: 1, paddingVertical: 10, alignItems: 'center', borderRadius: radius.full },
  tabActive: { backgroundColor: card, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 2, elevation: 2 },
  ...
  docLabel: { fontSize: 12, fontWeight: '600', color: mutedForeground, marginBottom: 8 },
  docBtn: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
