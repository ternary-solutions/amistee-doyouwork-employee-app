...
const HeaderOptionsContext = createContext<
  HeaderOptionsContextValue | undefined
>(undefined);
...
/** Call from screens that want to set title, subtitle, headerAction, showBack in the header. Clears on unmount. Options are only applied when the current route matches this screen (so still-mounted drawer screens can't overwrite the header). Use routePath for list screens so they don't overwrite the header when a child route (e.g. detail) is active.
 * @param options - Header options to apply
 * @param routePath - Optional. The route this screen owns (e.g. '/(app)/spiffs'). When provided, options apply only when pathname matches this path. Omit for detail screens so they apply on their nested route. */
export function useSetHeaderOptions(
  options: HeaderOptions | null,
  routePath?: string,
) {
  const pathname = usePathname();
  const { setOptions } = useContext(HeaderOptionsContext) ?? {};
  const effectivePath = routePath ?? pathname;
  useLayoutEffect(() => {
    setOptions?.(options, effectivePath);
    return () => setOptions?.(null, effectivePath);
  }, [setOptions, options, effectivePath]);
}
...
export function HeaderOptionsProvider({ children }: { children: ReactNode }) {
  ...
  const setOptions = useCallback(
    (o: HeaderOptions | null, callerPathname?: string) => {
      setOptionsState((prev) => {
        if (
          callerPathname != null &&
          normalizePath(callerPathname) !== normalizePath(pathname)
        )
          return prev;
        return o;
      });
