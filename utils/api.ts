...
export { getBaseUrl, getWebSocketNotificationsBaseUrl } from "@/utils/config";
export { getMediaSource, getMediaUrl } from "@/utils/mediaSource";
...
export async function apiRequest<Req, Res>(
  ...
  try {
    ...
    if (axiosError.response?.status === 401 && requireAuth && !isRetry) {
      try {
...
async function applyAuthResponse(data: LoginResponse): Promise<void> {
  ...
  const locationId =
    data.user.location_id ?? data.user.location_ids?.[0] ?? null;
...
export async function fetchMe(): Promise<User> {
  ...
  // Populate locations for any screen that needs a location picker
  const { locationsService } = await import("@/services/locations");
  try {
    const locationsRes = await locationsService.list(1, 100);
    useMainStore.getState().setLocations(locationsRes?.items ?? []);
  } catch (e) {
    console.warn("[fetchMe] Failed to load locations:", e);
  }
